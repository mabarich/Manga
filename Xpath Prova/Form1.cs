    ...
    public partial class Form1 : Form
    {
        ...
        private void checkOnStartUp()
        {
            try
            {
                ...
                if (ok)
                {
                    if (MessageBox.Show("Ho trovato un nuovo aggiornamento disponibile. Vuoi aggiornare ora?", "Aggiornamento trovato", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        var process = new Process();
                        process.StartInfo.FileName = "Updater.exe";
                        process.StartInfo.Arguments = ultimaModificaS;
                        process.Start();
                        Environment.Exit(0);
                    }
                }
        ...
        private bool isFav(string p)
        {
            ...
            if (favsa != null)
            {
                ...
                {
                    string nome=favsa[i].ToString().ToLower();
                    if(p.ToLower().Contains(nome))
                        found=true;
                }
            }
            return found;
        ...
        private void toolStripSplitButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Non ho trovato nessun aggiornamento disponibile.", "Aggiornamento non trovato", MessageBoxButtons.OK);
                }
            }
            catch(Exception){}
        }
